...
public class ArrCharOps {
     ...
     * sometimes referred to as the array's "hash code". Later in the course
     * we'll explain what these hash codes are used for. For now, simply
     * implement the specification given below. The hash code is computed as:
     * arr[0]*7^(n-1) + arr[1]*7^(n-2) + ... + arr[n-2]*7 + arr[n-1] where
     * arr[i] is the i'th character of the array, and n is the array's length.
    ...
    public static long hashCode(char[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            ...
            double pow = Math.pow(7, n);
            double iChar = arr[i];
            double multipltion = iChar * pow;
            hashCode += (int) multipltion;
        }
        return hashCode;
    }
